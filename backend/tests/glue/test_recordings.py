...
def test_recordings_the_frontend_has_no_card_for_are_left_out(
    settings: Settings, home: Path, tmp_path: Path
):
...
def test_nothing_is_scanned_without_authentication(open_settings: Settings):
    ...
    assert get_classified_recordings(
        open_settings, open_settings.destdir, NO_JOBS
    ) == RecordingClasses([], [], [])
...
def purge_path(
    home: Path,
    recording: str,
    running_jobs: frozenset[Path] = NO_JOBS,
    user_info: UserInfo | None = LECTURER,
) -> Path:
