...
def _is_in_domain(domain: str, validated: ValidatedEmail):
    return any(
        addr.endswith((f"@{domain}", f".{domain}"))
...
def generate_report(
    ...
    content = dedent(
        ...
        reason: {reason}
        """
    ).format(job_title=job_title, output=result.output_file, reason=message)
...
async def send_report(
    smtp_settings: SmtpSettings, recipient: str | None, job_title: str, result: Result
) -> None:
    """
    ...
    logger.debug(
        "SMTP through %s:%s as %s",
        smtp_settings.server,
        smtp_settings.port,
        smtp_settings.local_hostname,
    )
